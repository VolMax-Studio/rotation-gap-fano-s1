...
\begin{itemize}
    \item \textbf{$R_1$ ($W_1$ Mean Tolerance):} $|\bar{F}_{\text{recomputed}} - 2.42| < 0.005$ (corresponding to the half-width of the last printed digit).
    \item \textbf{$R_2$ ($W_1$ Standard Deviation):} $|s_{\text{recomputed}} - 0.36| \le 0.36$ (outer admissible spread band).
    \item \textbf{$R_5$ ($W_2$ Distance Stratification):} $|\bar{F}_{d} - F_{d,\text{reported}}| < 0.005$ for each distance $d \in \{3, 5, 7\}$.
