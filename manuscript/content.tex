...
& Aggregate mean ($\bar{F}$) and population standard deviation of $F_i$ ($\sigma_F$) & Transcribed from Author Script (§2) \\
& One-sample $t$-statistic against $F=1$ ($t = (\bar{F} - 1) / (\sigma_F / \sqrt{N})$) & Transcribed from Author Script (§2) \\
...
\begin{itemize}
    ...
    \item \textbf{$R_2$ (Verified with Limitations):} $0.005 \le |\bar{F}_{\text{recomputed}} - 2.42| \le 0.36$ ($0.36$ is the reported spread used as the outer admissible band).
...
$W_1$ & Population Std.~Dev. of $F_i$ ($\sigma_F$) & $0.36$ & $0.3620$ & $0.0020$ & (Descriptive / Part of $W_1$) \\
\hline
